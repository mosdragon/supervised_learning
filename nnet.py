...
# Neural Net Params
MAX_ITERATIONS = 100
HIDDEN_LAYER_PERCEPTRONS = 200  # Default 200
LEARNING_RATE = 0.01  # Default: 0.01
WEIGHT_DECAY = 0.00  # Default: 0.0
VERBOSE_OUTPUT = 0  # Default: 0
OUTPUT_LAYER = 'softmax'  # options: ['linear', 'softmax', 'tanh']
LOSS_FUNC = 'cross_entropy'  # options:  ['cross_entropy', 'square', 'crammer_singer']
...
'''
Implementation by Andreas Mueller: https://gist.github.com/amueller/2061456
'''
...
class BaseMLP(BaseEstimator):
    ...
    def fit(self, X, y, max_epochs, shuffle_data, verbose=0):
        ...
        if n_samples % self.batch_size != 0:
            warnings.warn("Discarding some samples: Sample size not divisible by chunk size.")
...
for n_iteration, fold_nums in enumerate(kf):
    ...
    classifier = NeuralNet(n_hidden=HIDDEN_LAYER_PERCEPTRONS,
        lr=LEARNING_RATE, l2decay=WEIGHT_DECAY, loss=LOSS_FUNC,
        output_layer=OUTPUT_LAYER, batch_size=BATCH_SIZE, verbose=VERBOSE_OUTPUT)

    classifier.fit(trnX, trnY, max_epochs=MAX_ITERATIONS, shuffle_data=True)
